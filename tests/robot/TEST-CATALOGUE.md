...
| `regression/issue_141_transport_state_ownership.robot` | `regression`, `issue-141`, `permanent`, `ui`, `speech`, `transport` | Exercises the real MainForm keyboard and button Play/Pause paths when monitoring has stopped but speech still owns an active retained position; requires no monitor restart/history reset/seek jump and no focus transfer to Theme. |
